...
proc pswChainCheck::run {args} {
    ...
    # Detect real directed cycles with iterative DFS. This is kept separate
    # from segment traversal: reaching an already reported segment is not, by
    # itself, proof of a cycle.
    array set color {}
    array set cycle_seen {}
    set cycles {}
    set cycle_nodes {}
    foreach start $inst_names {
        if {[info exists color($start)] && $color($start) != 0} { continue }
        set stack [list [list $start 0]]
        set active_path {}
        while {[llength $stack] > 0} {
            lassign [lindex $stack end] node child_index
            if {![info exists color($node)] || $color($node) == 0} {
                set color($node) 1
                lappend active_path $node
            }
            set children $succ($node)
            if {$child_index < [llength $children]} {
                set child [lindex $children $child_index]
                set stack [lreplace $stack end end [list $node [expr {$child_index + 1}]]]
                if {![info exists color($child)] || $color($child) == 0} {
                    lappend stack [list $child 0]
                } elseif {$color($child) == 1} {
                    set begin [lsearch -exact $active_path $child]
                    if {$begin >= 0} {
                        set one_cycle [lrange $active_path $begin end]
                        set cycle_key [join [lsort -dictionary $one_cycle] "|"]
                        if {![info exists cycle_seen($cycle_key)]} {
                            set cycle_seen($cycle_key) 1
                            lappend cycles $one_cycle
                            foreach item $one_cycle { lappend cycle_nodes $item }
                        }
                    }
                }
            } else {
                ...
                if {[llength $active_path] > 0 && [lindex $active_path end] eq $node} {
                    set active_path [lrange $active_path 0 end-1]
                }
            }
        }
    }
    set cycle_nodes [_unique $cycle_nodes]

    # Divide the graph into unbranched segments. A legal serial chain normally
    # appears as one segment from a root to an end port. A new segment starts
    # immediately after every branch, and at every root/merge.
    array set visited {}
    ...
    foreach inst_name $inst_names {
        if {[llength $pred($inst_name)] != 1} {
            lappend start_candidates $inst_name
        } else {
            set only_pred [lindex $pred($inst_name) 0]
            if {[llength $succ($only_pred)] != 1} {
                lappend start_candidates $inst_name
            }
        }
    }
    ...
    foreach start $start_candidates {
        ...
        while {1} {
            if {[info exists visited($current)]} {
                set termination "REJOINS_REPORTED_SEGMENT:$current"
                break
            }
    ...
    # Anything still unvisited belongs to a cycle or an unusual graph region.
    # Trace it as an additional segment so no PSW is omitted.
    foreach start $inst_names {
        if {[info exists visited($start)]} { continue }
        incr segment_no
        set sequence {}
    ...
    puts $report_fh "  Actual directed cycles       : [llength $cycles]"
    puts $report_fh "  PSW instances inside cycles  : [llength $cycle_nodes]"
    ...
    puts $report_fh "  CYCLE_NODES        : $cycle_nodes"
    set cycle_number 0
    foreach one_cycle $cycles {
        incr cycle_number
        puts $report_fh "  CYCLE_$cycle_number          : $one_cycle"
    }
    ...
    return [list \
        ...
        cycles [llength $cycles] \
        cycle_nodes [llength $cycle_nodes]]
}
